...
from .Search_resnet101_baseblock_softmax_sybn_dilated import ALLSearch_SE
...
class ALLSearch_Decode_SE(nn.Module):
    def __init__(self, weight,n_classes , dilation , deep_supervision = False, input_channels = 3 ):
        super().__init__()
        ...
        for layer in range(self.layers):
            if layer == 0:
                ...
                for stage in range(self.layers-layer):
                    if stage == 0:
                        self.layermodule.append(CBBlock(self.branchx1_0[0] * nb_filter[stage] + self.branchx1_0[1] * nb_filter[stage+1], nb_filter[stage],dilation = self.dilation[stage]))
                    
                    elif stage == 1:
                        self.layermodule.append(CBBlock(self.branchx1_1[0] * nb_filter[stage] + self.branchx1_1[1] * nb_filter[stage+1] + self.branchx1_1[2] * nb_filter[stage-1], nb_filter[stage],dilation = self.dilation[stage]))
                    elif stage == 2:
                        self.layermodule.append(CBBlock(self.branchx1_2[0] * nb_filter[stage] + self.branchx1_2[1] * nb_filter[stage+1] + self.branchx1_2[2] * nb_filter[stage-1], nb_filter[stage],dilation = self.dilation[stage]))
                        
                    else :
                        self.layermodule.append(CBBlock(self.branchx1_3[0] * nb_filter[stage] + nb_filter[stage+1] + self.branchx1_3[1] * nb_filter[stage-1] , nb_filter[stage],dilation = self.dilation[stage]))
                ...
                for stage in range(self.layers-layer):
                    if stage == 0: 
                        self.layermodule.append(CBBlock((self.branchx2_0[0] + self.branchx2_0[1]) * nb_filter[stage] + self.branchx2_0[2] * nb_filter[stage+1], nb_filter[stage],dilation = self.dilation[stage]))
                    
                    elif stage == 1:    
                        self.layermodule.append(CBBlock((self.branchx2_1[0] + self.branchx2_1[1] ) * nb_filter[stage] + self.branchx2_1[2] * nb_filter[stage+1] + self.branchx2_1[3]  * nb_filter[stage-1], nb_filter[stage],dilation = self.dilation[stage]))
                            
                    else :
                        self.layermodule.append(CBBlock((self.branchx2_2[0] + self.branchx2_2[1]) * nb_filter[stage] + nb_filter[stage+1] + self.branchx2_2[2] * nb_filter[stage-1] , nb_filter[stage],dilation = self.dilation[stage]))
                ...
                for stage in range(self.layers-layer):
                    if stage == 0:
                        self.layermodule.append(CBBlock((self.branchx3_0[0] + self.branchx3_0[1] + self.branchx3_0[2]) * nb_filter[stage] + self.branchx3_0[3] * nb_filter[stage+1], nb_filter[stage],dilation = self.dilation[stage]))
                    
                    else:    
                        self.layermodule.append(CBBlock((self.branchx3_1[0] + self.branchx3_1[1] + self.branchx3_1[2] ) * nb_filter[stage] + nb_filter[stage+1] + self.branchx3_1[3] * nb_filter[stage-1]  , nb_filter[stage],dilation = self.dilation[stage]))
                ...
                self.layermodule = nn.ModuleList()
                for stage in range(self.layers-layer):  
                    self.layermodule.append(CBBlock((self.branchx4_0[0] + self.branchx4_0[1] + self.branchx4_0[2] + self.branchx4_0[3]) * nb_filter[stage] + nb_filter[stage+1], nb_filter[stage],dilation = self.dilation[stage]))
...
def get_seg_model(cfg, **kwargs):
    ...
    model_pre = ALLSearch_SE(n_classes = cfg.DATASET.NUM_CLASSES, dilation = cfg.MODEL.DILATION,deep_supervision = cfg.MODEL.DS)
    model_pre.init_weights(cfg.MODEL.PRETRAINED)
    para = model_pre.arch_parameters()
    model = ALLSearch_Decode_SE(para , n_classes = cfg.DATASET.NUM_CLASSES, dilation = cfg.MODEL.DILATION ,deep_supervision = cfg.MODEL.DS)
